...
mongoose
  .connect(
    `mongodb+srv://${process.env.DB_CONN_USERNAME}:${process.env.DB_CONN_PASSWORD}@portfolio-dev.luexvtb.mongodb.net/?appName=portfolio-dev`,
  )
  .then((val) => {
...
app.use((error, req, res, next) => {
  return res.status(error.statusCode || 500).json({
    status: error.statusText || "rejected",
